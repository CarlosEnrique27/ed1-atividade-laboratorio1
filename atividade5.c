...
/* Exercício 1: Ordena o vetor trocando apenas ponteiros para manter o mais próximo em vetorEntidades[1] */
void ordenarPorDistancia(Vector2 posJogador) {
    for (int i = 1; i < totalEntidades; i++) {
        for (int j = i + 1; j < totalEntidades; j++) {
            float dxI = vetorEntidades[i]->pos.x - posJogador.x;
            float dyI = vetorEntidades[i]->pos.y - posJogador.y;
            float distI = sqrtf(dxI * dxI + dyI * dyI);

            float dxJ = vetorEntidades[j]->pos.x - posJogador.x;
            float dyJ = vetorEntidades[j]->pos.y - posJogador.y;
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        /* Exercício 1: Executa a ordenação por distância uma vez por quadro */
        if (totalEntidades > 1) {
            ordenarPorDistancia(jogador->pos);
        }
